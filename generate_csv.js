...
let csv = 'Año;Mes;Categoría;Presupuesto\n';
for (const [name, amt, mlist] of [...ingresos, ...gastos]) {
    for (const m of mlist) {
        const val_str = amt.toFixed(2).replace('.', ',');
        csv += `2026;${months[m-1]};${name};${val_str}\n`;
    }
}

fs.writeFileSync('budget.csv', csv, 'utf-8');
